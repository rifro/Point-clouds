...
**global** void k_kwantiseer_en_key(const Vec3f* **restrict** punten,
u32 N,
Kwantisatie Q,
u64* **restrict** keys_out)
{
...
**global** void k_permuteer_punten(const Vec3f* **restrict** punten_in,
const u32* **restrict** index_perm,
u32 N,
...
void k_voxeliseer_morton_launch(const Vec3f* d_points, u32 N,
Kwantisatie Q,
u64* d_keys,
...
auto k_init_indices = [] **global** (u32* idx, u32 n){
u32 i = blockIdx.x * blockDim.x + threadIdx.x;
if (i<n) idx[i]=i;
};
...
// 4) Permuteer punten → d_points_perm (coalesced read met index_perm)
