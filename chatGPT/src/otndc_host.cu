...
__global__ void k_oTNDCRays(
    const float* x, const float* y, const float* z, int N,
    OTSlot* slots, int maxSlots,
    ...
    AssenResultaat* out,const OTConfig& cfg)
{
    ...
    k_oTNDCRays<<<blocks,threads>>>(x,y,z,N,
        dslots, S,
    ...
    std::sort(vals.begin(), vals.end(),
        [](const V& A,const V& B){return A.score>B.score;});
