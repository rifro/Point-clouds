...
RegressieUitkomst d_lineaireRegressieInVlak(const float3* __restrict__ pts,
                                            ...
                                            const float3& n_current)  // huidige n (voor 3D lift)
{
    ...
    float Sx=0.f, Sy=0.f, Sxy=0.f, Sxx=0.f; uint32_t N=0;
    ...
    float worst[K]; int   worstIdx[K];
    #pragma unroll
    for (int k=0;k<K;++k){ worst[k]= -1.f; worstIdx[k]=-1; }
    ...
    for (uint32_t i=0; i<M; ++i) {
        ...
        if (r > worst[m]) { worst[m]=r; worstIdx[m]=i; }
    }
    ...
    for (uint32_t i=0; i<M; ++i) {
        bool skip=false;
        #pragma unroll
        for (int k=0;k<K;++k) if (i==(uint32_t)worstIdx[k]) { skip=true; break; }
        if (skip) continue;
...
__device__ __forceinline__
float3 d_planaireRegressie(const float3* __restrict__ pts,
                            const uint32_t* __restrict__ idx,
                            uint32_t M,
                            const float3& n_in,
